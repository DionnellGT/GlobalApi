...
API RESTful para una tienda en línea construida con NestJS. Gestiona PROJECTos, usuarios, autenticación, carrito de compras, favoritos y pagos con Transbank Webpay Plus, con soporte para carga de archivos y comunicación en tiempo real mediante WebSockets.
...
- **Gestión de PROJECTos**: CRUD completo de PROJECTos con soporte para imágenes, tallas, géneros, tags, precios y stock. Incluye paginación, filtros por género, precio mínimo/máximo, tallas y búsqueda por texto.
- **Carrito de compras**: Agregar, actualizar cantidad y eliminar PROJECTos del carrito. El sistema detecta si el mismo PROJECTo con la misma talla ya existe e incrementa la cantidad automáticamente. Validación de tallas disponibles por PROJECTo. Consulta del carrito de cualquier usuario por ID.
- **Favoritos**: Agregar y eliminar PROJECTos de favoritos por usuario. Endpoint para administradores que agrupa todos los PROJECTos con el conteo de usuarios que los tienen en favoritos, ordenados de mayor a menor, con búsqueda y paginación.
...
- **[OpenAI SDK](https://github.com/openai/openai-node)**: Integración con `gpt-4o-mini` para recomendaciones de PROJECTos relacionados. El prompt envía metadatos mínimos (título, tags, género, precio) para optimizar el uso de tokens. Se usa `node-fetch@2` como polyfill de `fetch` para compatibilidad con Node 17.
...
| Método | Ruta | Rol requerido | Descripción |
|--------|------|---------------|-------------|
| GET | `/` | — | Listar PROJECTos (filtros: género, precio, tallas, texto) |
| GET | `/:term` | — | Obtener PROJECTo por ID, slug o título |
| GET | `/:id/related` | — | PROJECTos relacionados sugeridos por IA (OpenAI) |
| POST | `/` | user | Crear PROJECTo |
| PATCH | `/:id` | admin | Actualizar PROJECTo |
| DELETE | `/:id` | admin | Eliminar PROJECTo |
...
| POST | `/` | user | Agregar PROJECTo al carrito |
...
| POST | `/` | user | Agregar PROJECTo a favoritos |
| DELETE | `/:PROJECTId` | user | Eliminar PROJECTo de favoritos |
| GET | `/admin/group` | admin | PROJECTos agrupados por conteo de favoritos |
...
| Método | Ruta | Rol requerido | Descripción |
|--------|------|---------------|-------------|
| POST | `/PROJECT` | — | Subir imagen de PROJECTo |
| GET | `/PROJECT/:imageName` | — | Obtener imagen de PROJECTo |
